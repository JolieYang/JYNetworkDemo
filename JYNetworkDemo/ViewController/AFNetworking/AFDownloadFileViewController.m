...
- (IBAction)startDownload:(id)sender {
    ...
    if ([selectedBtn.currentTitle isEqualToString:@"开始下载"]) {
        [self fileLengthAndTypeForURL:Download_URL];
    ...
    if (!_downloadTask) {
        ...
        _downloadTask = [self.manager downloadTaskWithRequest:request progress:^(NSProgress * _Nonnull downloadProgress) {
            ...
            if (!error) {
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    [self updateImageViewWithPath:filePath];
                });
            }
        }];
    }
...
- (void)updateImageViewWithPath:(NSURL *)filePath {
...
/**
 * 通过HEAD从response获取文件基本信息: 如文件大小，文件类型，文件名称
 */
- (void)fileLengthAndTypeForURL:(NSString *)urlString {
    if (!self.dataTask) {
